<template>
  <b-container fluid>
    <b-row>
      <b-col lg="4">
        <iq-card body-class="iq-card-body pl-0 pr-0 pt-4">
          <template v-slot:body>
            <div class="docter-details-block">
              <div class="docter-profile text-center">
                <b-img :src="user.profile" alt="profile-img" class="avatar-130 img-fluid rounded" />
              </div>
        ...
        <iq-card body-class="iq-card-body">
          <template v-slot:body>
            ...
            <div class="iq-card-body p-0 m-0">
              <div class="about-info" style="overflow-x: scroll;">
                <div class="row">
                  ...
                  <div class="col-7">{{ userRoleName }}</div>
                  <div class="col-5">Email:</div>
                  <div class="col-7"><a :href="`mailto:${ user.email }`"> {{ user.email }} </a></div>
                  <div class="col-5">Phone No.:</div>
                  <div class="col-7"><a :href="`tel:${user.phone}`">{{ user.phone ?? 'N/A' }}</a></div>
                  <div class="col-5">Mobile No.:</div>
                  <div class="col-7"><a :href="`tel:${user.mobile}`">{{ user.mobile ?? 'N/A' }}</a></div>
                  <div class="col-5">Location:</div>
                  <div class="col-7">{{ user.location }}</div>
                </div>
              </div>
            </div>
          </template>
  ...
  methods: {
    setUserPersonalInfo (data) {
      if (data?.profile_path) {
        this.user.profile = data?.profile_path
      }
      ...
      this.user.middlename = data?.role
      this.user.phone = data?.phone_num
      this.user.mobile = data?.mobile_num
      this.user.location = data?.address
