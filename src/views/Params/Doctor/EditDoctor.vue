          ...
          <b-col lg="3">
            <iq-card>
              <template v-slot:headerTitle>
                <h4 class="card-title">Update Dentist</h4>
              </template>
          ...
          <b-col lg="9">
            <iq-card>
              ...
              <template v-slot:body>
                <div class="new-doctor-info">
                  <b-row>
                    ...
                    <ValidationProvider name="License No." rules="required" v-slot="{ errors }">
                      <b-form-input v-model="doctor.license_no" :class="(errors.length > 0 ? ' is-invalid' : '')" type="text" name="license_no" id="license_no" placeholder="License No."></b-form-input>
                      <div class="invalid-feedback">
                          <span>{{ errors[0] }}</span>
                        </div>
                    </ValidationProvider>
                    ...
                    <b-form-group class="col-md-6" label="Email Address:" label-for="email_address">
                      <ValidationProvider name="Email Address" rules="required" v-slot="{ errors }">
                        <b-form-input v-model="doctor.email_address" type="email" placeholder="Email Address" :class="(errors.length > 0 ? ' is-invalid' : '')"></b-form-input>
                        <div class="invalid-feedback">
                          <span>{{ errors[0] }}</span>
                        </div>
                      </ValidationProvider>
                    ...
                    <b-form-group class="col-md-6" label="Contact No:" label-for="email_address">
                      <ValidationProvider name="Contact No." rules="required" v-slot="{ errors }">
                        <b-form-input v-model="doctor.contact_no" type="text" placeholder="Contact No." :class="(errors.length > 0 ? ' is-invalid' : '')"></b-form-input>
                        <div class="invalid-feedback">
                          <span>{{ errors[0] }}</span>
                        </div>
                      </ValidationProvider>
                ...
                <b-button variant="primary" type="submit">Update Doctor</b-button>
              </div>
              </template>
            </iq-card>
          </b-col>
  ...
  methods: {
    ...
    setDoctorInfo (data) {
      if (data?.avatar) {
        ...
        if (cnt.length > 3) {
          this.doctor.profile_image = data?.avatar
        } else {
          this.doctor.profile_image = process.env.VUE_APP_API_BASE_URL + data?.avatar
        }
      }
        this.doctor.isUploadImg = false
        this.doctor.first_name = data?.first_name
        this.doctor.middle_name = data?.middle_name
        this.doctor.last_name = data?.last_name
        this.doctor.address_line1 = data?.address_line1
        this.doctor.address_line2 = data?.address_line2
        this.doctor.birth_date = data?.birth_date
        this.doctor.height = data?.height
        this.doctor.weight = data?.weight
        this.doctor.sex = data?.sex
        this.doctor.civil_status = data?.civil_status
        this.doctor.job_title = data?.job_title
        this.doctor.license_no = data?.license_no
        // this.doctor.profile_image = data?.avatar ?? require('../../../assets/images/user/11.png')
        this.doctor.contact_no = data?.contact_no
        this.doctor.fb_account = data?.fb_account
        this.doctor.twitter_account = data?.twitter_account
        this.doctor.linkedin_account = data?.linkedin_account
        this.doctor.instagram_account = data?.instagram_account
        this.doctor.nationality = data?.nationality
        this.doctor.email_address = data?.email_address
    },
    ...
    updateDoctorRecord () {
      ...
      doctorService.updateDoctor(this.doctor, id).then(({ data }) => {
        if (data.success) {
          this.$swal(data?.message ?? 'Doctor Record was updated successfully!!!')
